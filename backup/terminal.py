...
class TerminalWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for y in range(self.rows):
            ...
            while x < self.cols:
                cell = line[x]
                style = self._style(cell, sel, x, y)
                run = [cell.data or " "]
                x2 = x + 1
                while x2 < self.cols:
                    nxt = line[x2]
                    if self._style(nxt, sel, x2, y) != style:
                        break
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            self.sel_anchor = self.sel_head = self._cell_at(event.position())
            self.update()
        self.setFocus()
    ...
    def mouseMoveEvent(self, event):
        if event.buttons() & Qt.LeftButton and self.sel_anchor:
            self.sel_head = self._cell_at(event.position())
            self.update()
    ...
    def mouseDoubleClickEvent(self, event):
        ...
        self.sel_anchor, self.sel_head = (start, y), (end, y)
        self.update()
